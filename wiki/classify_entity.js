...
const EntityClassifier = require('./entity_classifier')
/**
 ...
 * 下载JSON格式数据后使用JSONUtil.loadSubclassesSync加载子类信息用于判断实体类别
 */

// load classes resources
let locations = JSONUtil.loadSubclassesSync('../data/locations.json');
let organizations = JSONUtil.loadSubclassesSync('../data/organizations.json');
// read write streams
const rl = readline.createInterface({
  input: fs.createReadStream('../data/output-2020-12-28.jl')
})
// create write stream for per/org/loc
...
rl.on('line', (line) => {
  ...
  if (classifier.isOrganization(entity, organizations)) os_org.write(entityStr);
